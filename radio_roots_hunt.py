#!/usr/bin/env python3
"""
Radio Roots Hunt: Search for Erdos #993 counterexample using "Radio Roots" strategy.

Strategy derived from empirical analysis of log-concavity failures vs near-misses:
1.  **High Near-Miss Ratio (NM)**: The primary signal. We want deep "valleys" in the coefficient sequence.
2.  **Right-Shifted Roots (Max Re)**: Roots should be pushed towards the imaginary axis (Re -> 0).
3.  **Low Imaginary Parts (Max |Im|)**: Unlike LC failures (which have huge Im), dangerous near-misses have low Im.

Score = (NM_Ratio * 100) + (Max_Re * 10) - (Max_Im * 0.1)

If NM_Ratio is low (< 0.5), we rely more on the root geometry to guide us to the "danger zone".
If NM_Ratio is high, it dominates.
...
try:
    from indpoly import (
        is_unimodal, 
        poly_add, 
        poly_mul, # Use indpoly's numpy-accelerated version
        first_unimodality_break,
        near_miss_ratio
    )
except ImportError:
    # Fallback if indpoly not in path, but it should be
...
def radio_score(seq: List[int]) -> Tuple[float, float, float, float]:
    ...
    # 1. Near Miss Ratio (Primary Signal)
    nm, _ = near_miss_ratio(seq)
    
    # 2. Roots Analysis
    try:
        # Normalize coefficients to avoid overflow in np.roots
    ...
    # Independence roots typically have small modulus. 
    # If Re > 2.0, it's likely a numerical artifact for degree ~100.
    if max_real > 2.0:
        return (-9999.0, nm, max_real, -max_imag)
    
    # Composite Score
    # We want high NM.
    # We want max_real closest to 0 (it's usually negative, so maximize it).
    # We want max_imag to be small (but not zero).
    
    # Weighting heuristic:
    # NM is [0, 1+]. 1.0 is the threshold.
    # Max_Re is [-0.5, -0.01]. 
    # Max_Im is [0, 50+]. Dangerous ones are ~1.0.
...
def main():
    ...
    ap.add_argument("--out-dir", type=str, default="out_radio")
    args = ap.parse_args()
    
    print(f"Starting Radio Roots Hunt (seed={args.seed})...")
    ...
    while True:
        ...
        try:
            ...
            if b_re > 10.0:
                print("    [WARNING] Max Real > 10.0! Likely numerical instability.")
        except:
            print("    [Error] Could not re-calc roots.")
