"""
Profile cases where ⋆ FAILS but Karlin rescues (w_k ≥ 0 despite Term2+Term3 < 0).
Focus on tightest rescues (smallest Term1 / |Term2+Term3|).
"""
...
def main():
    ...
    # Collect tightest Karlin rescues: smallest Term1/|Term2+Term3| ratio
    # where Term2+Term3 < 0
    tightest_ratio = []  # max-heap by -ratio

    total_rescues = 0
    ratio_histogram = defaultdict(int)  # bucket by floor(ratio)
    ...
    for n in range(args.min_n, args.max_n + 1):
            ...
            for root in range(n):
                ...
                for step_idx, ci in enumerate(perm):
                    ...
                    for k in range(1, mode_I):
                        ...
                        if star_margin >= 0:
                            continue  # ⋆ holds, no rescue needed

                        # ⋆ fails — compute full 3-term
                        Ak = _coeff(A, k+1)
                        Akm = _coeff(A, k)
                        ...
                        if t23 >= 0:
                            continue  # curvature alone handles it

                        total_rescues += 1
                        if Term1 > 0:
                            ratio = Term1 / abs(t23)
                            bucket = int(ratio)
                            ratio_histogram[bucket] += 1

                            record = (ratio, n, g6, root, s, ell, k,
                                      step_idx, mode_I, Term1, t23, star_margin)

                            if len(tightest_ratio) < args.top_k:
                                heapq.heappush(tightest_ratio, (ratio, record))
                            elif ratio < tightest_ratio[0][0]:
                                heapq.heapreplace(tightest_ratio, (ratio, record))
        ...
        proc.wait()
        print(f"n={n}: done ({total_rescues} rescues so far)")

    # Sort by ratio ascending
    results = sorted([r for _, r in tightest_ratio], key=lambda x: x[0])

    print()
    print("=" * 90)
    print(f"TOP {args.top_k} TIGHTEST KARLIN RESCUES (smallest Term1/|Term2+Term3|)")
    print("=" * 90)
    print(f"{'ratio':>8} | {'n':>3} | {'root':>4} | {'s':>2} | {'ell':>3} | "
          f"{'k':>3} | {'mode':>4} | {'step':>4} | {'Term1':>12} | {'T2+T3':>12} | "
          f"{'⋆_margin':>10}")
    print("-" * 100)
    for rec in results:
        ratio, n, g6, root, s, ell, k, step, mode, t1, t23, sm = rec
        print(f"{ratio:>8.3f} | {n:>3} | {root:>4} | {s:>2} | {ell:>3} | "
              f"{k:>3} | {mode:>4} | {step:>4} | {t1:>12} | {t23:>12} | "
              f"{sm:>10}")

    print()
    print("Ratio histogram (floor):")
    for bucket in sorted(ratio_histogram.keys()):
        print(f"  [{bucket}, {bucket+1}): {ratio_histogram[bucket]}")
