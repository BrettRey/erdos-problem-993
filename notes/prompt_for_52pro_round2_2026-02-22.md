...
Goal: close symbolic proof gaps (not more computation).
...
Task:
1) Propose the strongest proof route to close all remaining symbolic gaps.
2) Give a lemma dependency graph.
3) For each missing lemma, provide a concrete proof strategy with key inequalities.
4) Flag where assumptions are insufficient and exactly what extra lemma is needed.
5) Provide a “first 3 prompts” plan I can run iteratively with you.

Additional constraints for this round:
- Use the exact symbols above; do not rename core objects.
- If you introduce a derived quantity, define it in one line.
- Any “equivalent reformulation” must be shown algebraically.
- When giving a proof strategy, separate:
  - exact identities
  - inequalities
  - domain/sign assumptions.

Output format:
1. Primary route (one route only, no branching in this section).
2. Lemma graph.
3. Missing lemma details.
4. Insufficiency flags.
5. First 3 prompts.
