...
### Analytic follow-up: a sharp subcase and a pendant-pair recurrence

Normalizing the corrected bridge gives

\[
\frac{b_2}{e_2}+\frac{b_4}{e_4}
+\frac{b_2b_4}{e_2e_4}
\le \frac{5\alpha+17}{27(\alpha-3)}
\quad\text{when }D<0. \tag{11}
\]

The \(b_2=0\) subcase has additional exact structure. Every blocked
defect-three independent set is maximal, and a blocked defect-four set \(S\)
satisfies \(\alpha(T-N[S])\le1\); otherwise one or two residual additions
would produce a defect-two set and hence an extension of \(S\) to a maximum
set. Since the residual is a forest, it is only
\(\varnothing\), \(K_1\), or \(K_2\).

On the live \(\alpha\in\{17,18,19\}\) window, the sharp-looking sublemma
\(5b_4\le e_4\) would close this branch because
\(1/5<7/27\le(5\alpha+17)/(27(\alpha-3))\). An exact audit of 149,239
nonisomorphic trees through \(n\le18\), restricted to
\(\alpha\ge4\) and deficiency at most five, finds 586 cases with
\(\alpha\ge5\) and \(b_2=0\), with no failure of either
\(5b_4\le e_4\) or the companion \(5b_3\le e_3\). Equality in the former
occurs exactly once, at \(K_{1,5}\). Of the 419 small-corpus cases with
\(D<0\), 387 have \(b_2=0\) and 32 have \(b_2>0\). The 116.45-second run is
recorded in `../scripts/audit_joint_density_subcases_20260904.py` and
`../results/joint_density_subcases_n18_20260904.json`.

Two tempting imports were killed exactly. First, the naive five-repair
injection obtained by exchanging against maximum sets fails on graph6
``Hs`?GGC``: it needs 30 distinct repair slots but its bipartite repair graph
has maximum flow 26, even though the global inequality survives at
\(5b_4=30<35=e_4\). Second, sequential Cohen--Macaulay theory supplies the
right pure-skeleton bookkeeping but not the inequality. A split (hence
chordal) family with one six-vertex maximum facet and \(m\) two-vertex facets
has \(b_2=0\), \(e_4=15\), and \(b_4=m\), violating
\(5b_4\le e_4\) for \(m\ge4\). The missing force is therefore specific to
the original graph being a tree, not merely to its independence complex being
vertex decomposable, shellable, or sequentially Cohen--Macaulay.

A pendant matched pair gives a more promising exact recursion. If \(T^+\) is
formed by attaching \(w-p-l\), if \(E\) is the maximum-facet-generated
enumerator, if \(Q\) is generated by maximum sets avoiding \(w\), and if
\(H=I(T-w)-Q\), then

\[
E^+=(1+x)E+xQ,\qquad B^+=(1+x)B+xH. \tag{12}
\]

In reversed coefficients,

\[
e_d^+=e_{d-1}+e_d+q_d,\qquad
b_d^+=b_{d-1}+b_d+h_d. \tag{13}
\]

When \(b_2^+=0\), the two fivefold inequalities propagate from the base tree
provided only

\[
5h_3\le e_2+q_3,\qquad 5h_4\le q_4. \tag{14}
\]

A rooted kill test over all 5,446 nonisomorphic base trees through order 14
and every relevant root finds 838 eligible extensions and zero failures of
either inequality in (14) or either propagated inequality. The \(h_4\) bound
is attained, so its constant has no observed slack. This 8.23-second result is
recorded in `../scripts/audit_pendant_p2_boundary_20260904.py` and
`../results/pendant_p2_boundary_n14_20260904.json`.

This is the next proof-sized target: prove or refute (14) under
\(b_1=b_2=h_2=0\). It is naturally aligned with the deficiency-zero and
deficiency-one quotient trees, which must have a matched leaf bag and hence a
pendant \(P_2\). It does not dispose of the \(b_2>0\) branch, where the full
joint density (11) remains necessary. Full derivation, source audit, killed
injection, commands, and hashes are in
...
- The \(b_2=0\) residual lemma and the pendant-\(P_2\) recurrences are exact. The fivefold global inequalities and the two root-conditioned boundary inequalities have only bounded computational support.

- Sequential Cohen--Macaulay and \(h\)-triangle theory organizes the pure-skeleton strata but cannot prove the fivefold bound: an explicit split/chordal family violates it. The naive local five-repair injection also has an exact 9-vertex obstruction.
