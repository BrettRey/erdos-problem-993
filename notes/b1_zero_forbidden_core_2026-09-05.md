...
Section 9 also proves a separate sufficient criterion for every finite forest
with \(\alpha\in\{17,18,19\}\): if
\(3(\alpha-3)b_4\le(\alpha-7)e_4\), then \(s_3^2>s_2s_4\).
It does not require the vertex-count window or \(b_1=0\).

Status: both results passed independent AI mathematical review, including
independent reconstruction of the exact finite certificates; see
[the review record](../reviews/review-board-20260906-014743/synthesis.md).
The Section 9 low-density theorem and its general blocked-shadow lemma are
now graph-level Lean-verified, including the explicit bridge from matching-bag
code counts to graph-set counts. A fresh local build and exact-statement/axiom
audit passed; see [the verification record](../formalization/depth3_low_density_20260906/STATUS.md).
The b1-zero tree-window theorem is now also graph-level Lean-verified; see
[its verification record](../formalization/depth3_b1_zero_20260906/STATUS.md).
The formal proof uses a sufficient union bound `4 b_4 <= e_4` in the last
case, proved over all 16,796 plane rooted forests with explicit graph/count
and evaluator bridges, followed by Section 9's criterion. It does not
formalize Section 7's sharper graph maximum or its special b3/b2 estimate.
No novelty claim or manuscript change.
...
Counts at negative set sizes are zero. Define the combined correction
\[
D=2e_3b_3+b_3^2-e_2b_4-e_4b_2-b_2b_4,
\]
so \(s_3^2-s_2s_4=(e_3^2-e_2e_4)+D\).
...
Assume \(r>0\); Section 4 handles \(r=0\) separately.
...
Every \(d\)-defect layer with \(0\le d\le\alpha\) has
\(e_d\ge\binom\alpha d>0\), by taking subsets of one maximum independent
set. This justifies the divisions and the strict conclusions below.
...
The graph throughout this section is a connected tree. Its parameters are
...
written proof, now independently AI-reviewed. The resulting tree-window
theorem is also Lean-verified via the weaker sufficient bound described
above; this particular sharper graph bound is not claimed Lean-verified.
Neither route is a novelty claim or a solution of Erdős #993. All 55 repository
...
For every finite simple forest with \(\alpha\ge4\), the existing
forest-poset representation supplies one more useful lemma, without assuming
\(b_1=0\):
