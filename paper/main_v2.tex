...
In every tree tested, the minimum slack is attained at a singleton, where Proposition~\ref{prop:singleton} applies.
Propositions~\ref{prop:singleton} and~\ref{prop:mixed-single} together cover all $S \subseteq H$ with at most one non-leaf heavy vertex; the remaining case $|S \cap H_{\mathrm{nl}}| \ge 2$ is open.
