...
I use the subdivision-contraction recurrence $I(T_e;\, x) = I(T;\, x) + x \cdot I(T/e;\, x)$ (known in the general graph setting) and give a tree-DP derivation tailored to mode analysis.
...
For the regime-adaptive hard-step diagnostics used in the reduction framework, I also complete the $n=25$ Modal frontier scans (all $256+256$ residue shards for the alpha and lambda dictionaries), obtaining
$\alpha_{\mathrm{front}}(25)=0.15739052900321043$ at class $(2,21)$ and
$\lambda_{\mathrm{front}}(25)=0.34114585595914226$ at class $(5,18)$, with gap
$\alpha_{\mathrm{front}}-\lambda_{\mathrm{front}}=-0.18375532695593183$.
...
\begin{enumerate}
  ...
  \item Authoritative completion of the $n=25$ regime-adaptive frontier diagnostics used by the hard-step reduction, including explicit witness classes and gap values (Section~\ref{sec:discussion}).
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.28\linewidth} p{0.14\linewidth} p{0.23\linewidth} p{0.27\linewidth}}
    ...
    Conjecture~A closure via mean bound & Conditional & mode-mean bridge $\mathrm{mode}\le\lceil\mu\rceil$ & \S\ref{sec:reduction} + App.~\ref{app:certificates} \\
    ...
    Regime-adaptive hard-step frontier snapshot ($n=25$) & Empirical & complete Modal shard dictionaries & \S\ref{sec:discussion}, App.~\ref{app:certificates} \\
    \bottomrule
  \end{tabular}
\end{table}
...
For general graphs this recurrence appears in the edge-subdivision literature (e.g., Theorem~20 of \textcite{trinks2014}); I include the tree decomposition here because it is the algebraic starting point for the mode and mean analyses that follow.
...
\begin{remark}[Mean shift approach]\label{rem:mean-shift}
  ...
  I verified $|\mu(T) - \mu(T/e)| < 0.537$ for all 3\,348\,674 edges in trees on $3 \le n \le 18$ (exhaustive maximum $\approx 0.533$); in parameterised family scans, the largest observed value is $|\delta\mu| \approx 0.537$ at the tripod star $T(5,5,5)$ (centre-to-bridge edge).
  ...
  A strictly log-concave sequence of positive reals ($a_k^2 > a_{k-1} a_{k+1}$ at every interior index) has a unique mode, but mode $\in \{\lfloor\mu\rfloor, \lceil\mu\rceil\}$ does not follow from strict log-concavity alone (counterexample: $1, 2, 3, 4$).
  By \textcite{darroch1964}, this mode-localisation does hold for Poisson-Binomial distributions (coefficients of products $\prod(1 + p_i x)$); I verified it for all tree IS polynomials through $n = 22$ (zero violations among 9\,114\,283 trees).
  ...
  Proving $|\mu(T) - \mu(T/e)| < 1$ would therefore establish ECMS for any tree whose IS polynomial satisfies the mode-localisation property.
\end{remark}
...
Strict log-concavity implies a unique mode, and the mode-mean bound $\mathrm{mode} \le \lceil\mu\rceil$ holds computationally for all 9\,114\,283 trees through $n = 22$ (zero violations).
...
\begin{remark}
The mode-mean bound $\mathrm{mode}(I(T)) \le \lceil \mu(T) \rceil$ holds for all 931\,596 $\dleaf \le 1$ trees through $n = 23$ (zero violations), and for all 9\,114\,283 trees (of any type) through $n = 22$.
It also holds for the two log-concavity-failing trees at $n = 26$ (where $\mu \approx 7.83$ and $\mathrm{mode} = 8$), checked individually.
The bound appears to hold universally regardless of log-concavity.
...
The stronger localisation $\mathrm{mode} \in \{\lfloor\mu\rfloor, \lceil\mu\rceil\}$ (\textcite{darroch1964} proved this for Poisson-Binomial distributions) also holds for all 175\,722 $\dleaf \le 1$ trees through $n = 21$: 69.65\,\% have $\mathrm{mode} = \lfloor\mu\rfloor$ and 30.35\,\% have $\mathrm{mode} = \lceil\mu\rceil$.
This mode-localisation does not follow from log-concavity alone (the sequence $1, 2, 3, 4$ is strictly log-concave with unique mode~3 and mean~2), but it appears to hold for all tree IS polynomials.
\end{remark}
...
This complements earlier eventual-unimodality results for fixed cores with sufficiently many pendant attachments at one vertex \citep{galvinhilyard2018}, by adding an explicit asymptotic near-miss rate.
...
The proved bound $\mu < n/3$ (Corollary~\ref{cor:mu-bound}) settles Conjecture~A modulo the mode-mean inequality $\mathrm{mode} \le \lceil\mu\rceil$, which is verified for all 9\,114\,283 trees through $n = 22$ but remains open algebraically.
...
The remaining remote component decays geometrically with distance from the contracted edge at rate $\approx 1/\varphi^2 \approx 0.382$ per hop (where $\varphi$ is the golden ratio), and is empirically less than $0.264$ in absolute value across all $3\,348\,674$ edges in trees on $n \le 18$.
...
The proved mean bound $\mu < n/3$ reduces this to the mode-mean inequality $\mathrm{mode} \le \lceil\mu\rceil$; the extremal spider $S(2^k, 1)$ has gap $n/3 - \mu \to 1/6$, so the mean bound is tight.
...
An alternative algebraic approach decomposes the IS polynomial at any degree-2 vertex in a $\dleaf \le 1$ tree, expressing $I(T)$ as a linear combination of subtree polynomials whose log-concavity is inherited from the product structure; the mode bound then reduces to local coefficient inequalities on these factors, which hold computationally for all 931\,596 trees through $n = 23$ but remain open.
A third approach exploits the product structure of the DP more directly. At a support vertex~$r$, the exclude-root polynomial $E = \prod (E_c + x J_c)$ is built by iterated convolution with factors from child subtrees, while $J = \prod E_c$. Individual factors can fail ratio dominance ($E_c + x J_c \not\succeq E_c$ for roughly $30\%$ of subtrees at $n = 28$), yet the accumulated product satisfies $E \succeq J$ at every support vertex of every tree through $n = 22$ (907\,million checks, 0~failures), though the full ratio dominance fails in the tail ($k > \mathrm{mode}$) at $n = 32$. At each incremental step, the ratio-dominance minor $d_k(E_{\mathrm{new}}, J_{\mathrm{new}})$ splits into a \emph{Karlin term} $d_k(E_{\mathrm{acc}} \cdot g,\, J_{\mathrm{acc}} \cdot g)$, which is non-negative by the TP2 convolution theorem (since $E_{\mathrm{acc}} \succeq J_{\mathrm{acc}}$ inductively and $g = E_t$ is log-concave), and a correction term involving $h = xJ_t$. Empirically the Karlin term dominates the correction at every support vertex through $n = 20$; the case $s = 1$ (one non-leaf child, handling $63\%$ of support vertices) is proved. The discovery that aggregate ratio dominance persists despite factor-level failures was guided by an analogy with homeostatic property cluster theory \citep{boyd1999}: the DP product mechanism maintains an invariant that no individual component satisfies.
...
\texttt{paper-v2-2026-02-23-doi3} at commit
\texttt{05385c969f3069d3a5db31d009b71c397ef4c096}, released at
\href{https://github.com/BrettRey/erdos-problem-993/releases/tag/paper-v2-2026-02-23-doi3}{github.com/BrettRey/\allowbreak erdos-problem-993/\allowbreak releases/\allowbreak tag/\allowbreak paper-v2-2026-02-23-doi3}.
...
The corresponding Zenodo record is
\href{https://doi.org/10.5281/zenodo.18745547}{doi:\allowbreak 10.5281/\allowbreak zenodo.18745547}
(concept DOI
...
For the deterministic machine-artifact pipeline introduced in the final reduction rounds, the repository now includes executable runtimes
\texttt{pi\_n/} and \texttt{orchestrator\_v13/}, together with fixture/golden conformance tests (\texttt{T0..T7}) and replay verification.
Two committed artifact bundles are provided:
\texttt{results/orchestrator\_v13\_run\_n24\_n25/} (mixed-source normalization) and
\texttt{results/orchestrator\_v13\_run\_direct\_lambda\_n22\_n25/} (direct lambda-frontier extraction).
The direct lambda run is generated via
\texttt{scripts/build\_orchestrator\_input\_from\_lambda\_frontiers.py}
and replay-verified with
\texttt{python3 -m orchestrator\_v13.cli --verify}.
For theorem-facing machine artifacts, the direct lambda-frontier extraction path is the canonical source; the mixed-source normalization bundle is retained as an integration test only.

The machine-artifact serialization contract is:
\begin{enumerate}
  \item finite floats are emitted as JSON numbers; infinities as \texttt{\"inf\"}/\texttt{\"-inf\"}; NaN is forbidden;
  \item all artifact JSON is canonicalized with sorted keys and compact separators;
  \item each artifact file is replay-checked by re-running the deterministic pipeline on the same input bundle;
  \item integrity digests are SHA-256 hashes of canonical artifact bytes and can be recomputed independently.
\end{enumerate}
...
\begin{table}[ht]
  ...
  \begin{tabular}{l r r}
    ...
    $|\delta\mu| < 0.537$ & \liningnums{3\,348\,674} edges & $\le 18$ \\
    LC ($\dleaf \le 1$ trees) & \liningnums{931\,596} trees & $\le 23$ \\
    $\mathrm{mode} \le \lceil\mu\rceil$ (all trees) & \liningnums{9\,114\,283} trees & $\le 22$ \\
    ...
    Tie-fugacity: $\mu(\lambda_m) \ge m-1$ (all trees) & \liningnums{9\,114\,283} trees & $\le 22$ \\
    Bridge mode bound: $\mathrm{mode}(P) \ge m-1$ ($\dleaf \le 1$) & \liningnums{931\,596} trees & $\le 23$ \\
    Regime frontier shards (alpha + lambda, Modal) & \liningnums{512} shards & $n = 25$ \\
    \bottomrule
  \end{tabular}
\end{table}
