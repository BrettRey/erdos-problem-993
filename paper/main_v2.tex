...
I prove that the mean independent-set size satisfies $\mu(T) < n/3$ for every tree in which each vertex has at most one leaf-child ($\dleaf \le 1$), via a decimation identity and Steiner peeling on a heavy-vertex compensation function.
A chain of structural lemmas~-- Hub Exclusion and Transfer~-- reduces the mode bound for all trees to a single conjecture (Conjecture~A) about these $\dleaf \le 1$ trees; conditional on the mode-mean inequality $\mathrm{mode} \le \lceil\mu\rceil$, the mean bound settles Conjecture~A in that regime.
Separately, I re-derive the subdivision-contraction identity $I(T_e) = I(T) + x \cdot I(T/e)$ in tree DP form and show that, conditional on a conjectured Edge Contraction Mode Stability property together with a verified tail condition, any minimal counterexample is homeomorphically irreducible.
Neither conditional route by itself closes the full conjecture.
On the computational side, I verify the conjecture exhaustively for all 8\,691\,747\,673 trees on $n \le 29$ vertices, with log-concavity and near-miss metrics through $n = 28$.
An evolutionary search identifies multi-arm stars as the extremal family; the near-miss ratio satisfies $\nm(s) = 1 - C/s + O(1/s^2)$ with $C \in [4, 8)$.
\end{abstract}
...
Accordingly, the main text is organised around this theorem-first backbone: Section~\ref{sec:reduction} carries the primary proved result, while the subdivision route and computational frontier evidence are presented as supporting structural and falsification layers.
...
Think of each heavy vertex $h$ as carrying a deficit demand $P(h)-1/3$ and each neighbouring core vertex as carrying a non-negative supply term from the decimation identity (full weight in $C\setminus A$, half weight in~$A$).
The Steiner tree on $S \subseteq H_{\mathrm{core}}$ isolates a leaf-heavy vertex whose demand can be paid by private neighbours using only local edge inequalities.
...
This tree is the global minimiser across all tree types through $n = 22$, not only among $\dleaf \le 1$ trees.
...
\texttt{paper-v2-2026-03-04-doi5} at commit
\texttt{7615ec330d4da68351bee3f52a3f0ce5d1d85425}, released at
\href{https://github.com/BrettRey/erdos-problem-993/releases/tag/paper-v2-2026-03-04-doi5}{github.com/BrettRey/\allowbreak erdos-problem-993/\allowbreak releases/\allowbreak tag/\allowbreak paper-v2-2026-03-04-doi5}.
...
(version DOI:
\href{https://doi.org/10.5281/zenodo.18866031}{10.5281/\allowbreak zenodo.18866031}).
