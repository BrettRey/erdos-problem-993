...
A chain of structural lemmas~-- Hub Exclusion and Transfer~-- reduces the mode bound for all trees to a single conjecture (Conjecture~A) about these $\dleaf \le 1$ trees; the mean bound settles Conjecture~A conditionally, subject to a conjectured mode-mean inequality $\mathrm{mode} \le \lceil\mu\rceil$ that is verified for all 23~million trees through $n = 23$.
Independently, I re-derive the subdivision-contraction identity $I(T_e) = I(T) + x \cdot I(T/e)$ in tree DP form and show that a conjectured Edge Contraction Mode Stability property (verified for 24.7~million edges) implies any minimal counterexample is homeomorphically irreducible.
...
The full ratio $i_{j+1}/i_j$ exceeds~1 below the mode and is at most~1 at the mode; the sub-dominant term $\tbinom{k}{j-1}$ shifts the crossover by less than one unit, placing the mode at $m = \lfloor(2k+1)/3\rfloor$ (verified for all $k \le 600$; for large~$k$ the sub-dominant contribution is $O(1/2^m)$ relative to the dominant term).
...
By \textcite{darroch1964}, $\mathrm{mode}(A_j) \in \{\lfloor\mu_j\rfloor, \lceil\mu_j\rceil\}$.
...
Together: $F > 0$ for all $k \ge 6$ and $m \le k+2$, and $F = T_1 > 0$ for $m > k+2$ (verified for $k \le 3000$, $j \le 120$; for $k > 3000$ the hub-on weight is negligible and $A_j$ is dominated by the $(1+x)^j$ factor whose LC defects are increasing at the mode).
...
The table records per-$n$ wall times through $n = 27$; the dict-backed Modal extensions to $n = 28$ and $n = 29$ are summarized in the paragraph below.
...
A subsequent dict-backed Modal run extends the exhaustive frontier to $n = 28$ (2\,023\,443\,032 trees; artifacts \texttt{results/analysis\_n28\_modal\_unimodality.json} and \texttt{results/analysis\_n28\_modal\_lc\_nm.json}), again with no unimodality violations.
...
A further dict-backed Modal run extends the exhaustive frontier to $n = 29$ (5\,469\,566\,585 trees; artifact \texttt{results/analysis\_n29\_modal\_unimodality.json}), again with no unimodality violations.
...
An auxiliary Lean~4.28 formalization also sharpened the status of one possible algebraic closure route. The naive abstract claim that the STP2 ratio-positivity condition is preserved under convolution is false under its original hypothesis list: an internal-zeros counterexample satisfies nonnegativity, log-concavity, and pairwise STP2, yet its convolution fails STP2. On the positive side, the formalization proves the identity case and the single-leaf case. So the remaining obstacle is not the leaf step itself, but identifying the additional tree-specific support regularity needed for a valid general closure theorem.
...
The repository also includes a Lean~4.28 formalization of the local algebraic lemmas in \texttt{Formal/}. Bounded proof-completion runs using Aristotle \parencite{achim2025aristotle} were used for auxiliary formalization tasks; the current formalization leaves the general STP2 closure step open but records the proved leaf special case and the internal-zeros obstruction.


\section*{Acknowledgements}

Claude Opus~4.6 (Anthropic) assisted with computational exploration, code development, and manuscript drafting.
ChatGPT~5.2~Pro and Codex~5.3 (OpenAI) contributed to proof-strategy exploration.
...
The $n = 28$ and $n = 29$ extensions used dict-backed Modal runs with 1024 workers, merged locally via \texttt{scripts/collect\_modal\_results.py}; missing residues were repaired with a \texttt{dispatch\_missing} backfill, and the final $n = 29$ tail shard was independently checked via an odd-modulus rescue split before the main dict closed.
...
# n = 28 (Modal dict-backed, 1024 workers)
...
# n = 29 (Modal dict-backed, 1024 workers)
...
\texttt{pi\_n/} and \texttt{orchestrator\_v13/} with fixture-based conformance tests and replay verification.
...
\begin{table}[ht]
  ...
  \begin{tabular}{l r r r}
    ...
    $\mathrm{mode} \le \lceil\mu\rceil$ (all trees) & \liningnums{9\,114\,283} trees & $\le 22$ & \S\ref{subsec:mean} \\
