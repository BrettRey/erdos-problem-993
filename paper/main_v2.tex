...
I prove $\mu(T) < n/3$ for every tree with $\dleaf \le 1$ via Steiner peeling on a compensation function.
Hub Exclusion and Transfer reduce the mode bound for all trees to a single conjecture about $\dleaf \le 1$ trees; conditional on $\mathrm{mode} \le \lceil\mu\rceil$, the mean bound settles this conjecture.
A subdivision-contraction identity shows that, conditional on a conjectured Edge Contraction Mode Stability property and a verified tail condition, any minimal counterexample is homeomorphically irreducible.
...
Multi-arm stars are the extremal family, with near-miss ratio $\nm(s) = 1 - C/s + O(1/s^2)$, $C \in [4, 8)$.
\end{abstract}
...
More broadly, AI-assisted search is now also producing new extremal constructions outside the tree setting: \textcite{nagda2026ramsey} report improved lower bounds for several classical Ramsey numbers via AlphaEvolve.
On the algebraic side, \textcite{bendjeddou2024} proved that a subclass of trees~-- those in the image of an edge-replacement operator~-- have \enquote{pre-Lorentzian} multivariate independence polynomials, hence log-concave (and unimodal) independence sequences, but the full conjecture remains open.
...
\textcite{li2025spiders} proved that all spiders have log-concave independence polynomials, implying unimodality for brooms as a special case.
...
\begin{enumerate}
  ...
  \item A tree-specialized derivation and mode-analysis use of the subdivision-contraction recurrence $I(T_e) = I(T) + x \cdot I(T/e)$ (Section~\ref{sec:identity}).
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.28\linewidth} p{0.14\linewidth} p{0.23\linewidth} p{0.27\linewidth}}
    \toprule
    Claim & Status & Depends on & Evidence path \\
    \midrule
    Reduction to $\dleaf \le 1$ regime (Hub Exclusion + Transfer) & Proved & PNP framework lemmas & \S\ref{sec:reduction} \\
...
Conjecture~\ref{conj:mode-mean} holds for all 23\,942\,357 trees through $n = 23$ (zero violations). This verification requires exact multiprecision arithmetic for the mean, so it is computationally heavier than the modular unimodality check and stops at $n=23$. It also holds, checked individually, for the two log-concavity-failing trees at $n = 26$ (where $\mu \approx 7.83$ and $\mathrm{mode} = 8$).
The stronger localisation $\mathrm{mode} \in \{\lfloor\mu\rfloor, \lceil\mu\rceil\}$ (\textcite{darroch1964} proved this for Poisson-Binomial distributions) also holds for all 175\,722 $\dleaf \le 1$ trees through $n = 21$.
...
\paragraph{Steiner-peeling intuition.}
Think of each heavy vertex $h$ as carrying a deficit demand $P(h)-1/3$ and each neighboring core vertex as carrying a non-negative supply term from the decimation identity (full weight in $C\setminus A$, half weight in~$A$).
The Steiner tree on $S \subseteq H_{\mathrm{core}}$ isolates a leaf-heavy vertex whose demand can be paid by private neighbors using only local edge inequalities.
Peeling that Steiner leaf removes one heavy vertex while preserving positivity of the remaining budget, so induction closes once every heavy vertex has been peeled.
...
Then $h$ has $k = \deg_T(h) - 1 \ge 1$ private $C$-neighbors $u_1, \ldots, u_k$ and one non-private neighbor $v_0$.
...
For $k = 1$ (so $\deg_T(h) = 2$, with sole private $A$-neighbor $u_1$ and non-private $v_0$), the belief-propagation fixed-point equations give
\[
...
It therefore suffices to check that $i_m \ge i_{m-1}$ and $i_m \ge i_{m+1}$ for $m = \lfloor(2k+1)/3\rfloor$.
...
\]
The last inequality is strict unless $r = 1$ (that is, $k = 5$), where $i_3 = i_4$ and the leftmost mode is still $m = 3$.
Hence the leftmost mode is $m = \lfloor(2k+1)/3\rfloor$ for all $k \ge 3$.

\textit{Part~3.}
The condition $kq_m > m-1$ is equivalent to $\lambda_m > \tau$, where $\tau = (m-1)/[2(k-m+1)]$.
...
\begin{proposition}[Mixed-spider tie-fugacity; computer-assisted]\label{prop:mixed-spider-tiefug}
...
When $m > k+2$, these binomial coefficients vanish and $F = T_1$; for $m \le k+2$ and $k \le 200$, exact integer computation over all 14\,430 active $(k,j)$ pairs confirms $F > 0$ with minimum ratio $T_1/|T_2| = 2.24$ at $(k,j) = (6,1)$; for $k > 200$, the ratio $T_1/|T_2|$ exceeds $10^{36}$ (growing exponentially in~$k$).
Together: $F > 0$ for all $k \ge 6$ and $m \le k+2$, and $F = T_1 > 0$ for $m > k+2$ (verified for $k \le 3000$, $j \le 120$; for $k > 3000$, the same hub-on estimate as in Proposition~\ref{prop:spider-tie-margin} gives a correction of order $O(k(5/7)^k)$, so the sign is governed by the positive hub-off term~$T_1$).
...
The exact scan artifacts cited in this proof are archived as \texttt{results/whnc\_mixed\_spider\_mode\_formula\_k6\_3000\_j120.json}, \texttt{results/subclaimA\_lane2\_envelope\_k3000\_j120\_v2.json}, and \texttt{results/subclaimA\_lane2\_lambda\_monotone\_k3000\_j120.json}.
\end{proof}
...
\begin{remark}[Mean shift approach]\label{rem:mean-shift}
  ...
  By \textcite{darroch1964}, mode localisation $\mathrm{mode}\in\{\lfloor\mu\rfloor,\lceil\mu\rceil\}$ holds for Poisson-binomial sequences; I verify the same localisation for all tree IS polynomials through $n=22$.
  Therefore, a proof of $|\mu(T)-\mu(T/e)|<1$ together with this localisation would imply ECMS.
\end{remark}
...
\begin{proof}
  ...
  If $|p-q|=1$, there is only one transition index between $m_-$ and $m_+$, so no strict valley can occur.
\end{proof}
...
The case $n = 26$ is the first at which log-concavity fails: \textcite{kadrawi2023} found exactly two failures through exhaustive enumeration.
...
Completeness was checked by requiring every expected shard key and matching the merged totals to the full OEIS tree count; the shard-level repair details are recorded in Appendix~\ref{app:repro}.
...
The proved bound $\mu < n/3$ (Corollary~\ref{cor:mu-bound}) settles Conjecture~A modulo Conjecture~\ref{conj:mode-mean} (the mode-mean bound), which is verified for all 23\,942\,357 trees through $n = 23$ but remains open algebraically.

\subsection{Conditional closure path}

Two main open problems remain.
First, prove ECMS.
The mean shift approach (Remark~\ref{rem:mean-shift}) reduces this to showing $|\mu(T) - \mu(T/e)| < 1$, with the local component proved to lie in $(0, 1/2)$ and the distance-1 remote contribution bounded by $|S_1| < 0.355$ (Proposition~\ref{prop:s1-formula}); for further empirical diagnostics, see Appendix~\ref{app:certificates}.
...
I archive a reproducible manuscript snapshot as Git tag
\texttt{paper-v2-2026-03-18-doi} at commit
\texttt{487096954fef98f7ff93a352d241f3f8d62ef0e5}, released at
\href{https://github.com/BrettRey/erdos-problem-993/releases/tag/paper-v2-2026-03-18-doi}{github.com/BrettRey/\allowbreak erdos-problem-993/\allowbreak releases/\allowbreak tag/\allowbreak paper-v2-2026-03-18-doi}.
API documentation is published at
\href{https://brettrey.github.io/erdos-problem-993/docs/}{brettrey.github.io/\allowbreak erdos-problem-993/\allowbreak docs/}.
...
The repository also includes a Lean~4.28 formalization of the local algebraic lemmas in \texttt{Formal/}. Bounded proof-completion runs using Aristotle \parencite{achim2025aristotle} were used for auxiliary formalization tasks; the current formalization leaves the general convolution-closure step open but records the proved leaf special case and an explicit obstruction to the naive abstract hypothesis set.


\section*{Acknowledgements}

Claude Opus~4.6 (Anthropic) assisted with computational exploration, Python code development, and prose drafting.
ChatGPT~5.2~Pro and Codex~5.3 (OpenAI) assisted with proof-strategy exploration, Lean formalization support, and editorial revision suggestions.
Gemini~3~Pro (Google) assisted with computational exploration.
Aristotle (Harmonic) was used only for bounded Lean proof-completion tasks in the \texttt{Formal/} artifact and did not supply the paper's main mathematical claims.
...
modal run analyze_modal_lc_nm.py --n 27 --workers 1024 --top-k 200 --lc-top-k 200
...
For the $n=25$ frontier snapshot (all shards complete), the aggregated values are
$\alpha_{\mathrm{front}}(25)=0.15739052900321043$ (witness class $(2,21)$),
$\lambda_{\mathrm{front}}(25)=0.34114585595914226$ (witness class $(5,18)$),
and gap $-0.18375532695593183$; see
\texttt{results/n25\_modal\_frontier\_authoritative\_2026-03-03.json}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.28\linewidth} p{0.25\linewidth} p{0.14\linewidth} p{0.23\linewidth}}
    \toprule
    ...
    Regime hard-step frontier completeness at $n=25$ & Full shard completion ($256+256$) + replayed aggregation & Complete; obstruction gap widened & \texttt{results/n25\_modal\_frontier\_authoritative\_2026-03-03.json} \\
    Machine-artifact determinism & Fixture matrix T0--T7 + golden-byte replay checks & Passing & \texttt{orchestrator\_v13/tests/} \\
    \bottomrule
  \end{tabular}
\end{table}
