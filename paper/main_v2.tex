...
Write $P(v)$ for the probability that vertex~$v$ belongs to a uniformly random IS (the scalar occupation probability, not to be confused with the polynomial~$P_v(x)$ of Section~\ref{sec:definitions}), and $\mu = \sum_v P(v)$ for the expected IS size.
...
Their occupation probabilities tend toward the infinite-path limit $P \approx 0.276 < 1/3$, but a proof of the global bound remains open.
...
The multi-arm star champion $M(s;\, 5, 5, 4, 2)$ achieves $C \approx 3.98$, slightly below the broom optimum~-- but the theorem gives $C \ge 4$ for any fixed core, so the margin remains positive for every finite~$s$.
...
Any counterexample must be homeomorphically irreducible (if ECMS holds), must have high leaf-degree vertices (by the PNP reduction), and cannot arise from pendant-leaf attachment to a fixed core (by the asymptotic theorem).
