...
On the computational side, I verify the conjecture exhaustively for all 3\,222\,181\,088 trees on $n \le 28$ vertices, with log-concavity and near-miss metrics through $n = 28$.
...
\begin{enumerate}
  ...
  \item Exhaustive verification for all trees on $n \le 28$ vertices, with log-concavity and near-miss metrics through $n = 28$ (Section~\ref{sec:exhaustive}).
...
\section{Exhaustive verification through \texorpdfstring{$n = 28$}{n = 28}}\label{sec:exhaustive}
...
The table records per-$n$ wall times through $n = 27$; the dict-backed Modal extension to $n = 28$ is summarized in the paragraph below.
...
This raises the exhaustive unimodality frontier to 3.22~billion trees through $n = 28$.
...
The $n = 28$ extension used dict-backed Modal runs with 1024 workers, merged locally via \texttt{scripts/collect\_modal\_results.py}; missing residues were repaired with a \texttt{dispatch\_missing} backfill.
...
\begin{table}[ht]
  ...
  \begin{tabular}{l r r r}
    ...
    \midrule
    Unimodality (exhaustive) & \liningnums{3\,222\,181\,088} trees & $\le 28$ & Conj.~\ref{conj:main} \\
