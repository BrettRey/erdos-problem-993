...
Finally, I develop an augmented injection framework for proving unimodality: every maximal independent set below the mode admits a ``swap'' to a larger set (removing one vertex, adding two private neighbors), and the resulting augmented bipartite graph satisfies Hall's condition for all trees on $n \le 18$ vertices.
\end{abstract}
...
\begin{enumerate}
  ...
  \item An augmented injection framework~-- combining containment and swap edges~-- that reduces unimodality to a Private Neighbor Property, verified for all trees on $n \le 18$ vertices (Section~\ref{sec:discussion}).
\end{enumerate}
Note added. Since this work was completed, Li, Li, Yang, and Zhang proved that all spiders are strongly log-concave, which implies log-concavity (hence unimodality) for brooms as a special case \citep{li2025spiders}. I include the elementary $s \ge p$ proof as a self-contained argument that aligns with the asymptotic analysis.
...
The optimizer consistently converged to a generalization of brooms that I call \emph{multi-arm stars}: a central hub vertex with $s$ pendant leaves and $k \ge 2$ paths of lengths $a_1, \dotsc, a_k$ emanating from the hub (total $n = 1 + s + \sum a_i$ vertices).
A standard broom is the special case $k = 1$.
...
\subsection*{Toward a proof: augmented injection}

The computational evidence raises a natural question: can unimodality be proved by injecting level~$k$ into level~$k+1$ for each $k$ below the mode?
The obvious approach~-- containment injection, where each size-$k$ independent set $S$ maps to $S \cup \{v\}$ for some vertex~$v$~-- fails because maximal independent sets below the mode cannot be extended.
At $n = 14$, roughly 54\% of trees have at least one such obstruction.

Adding \emph{swap edges} resolves this.
A swap edge connects $S$ at level~$k$ to $T = (S \setminus \{u\}) \cup \{v, w\}$ at level~$k+1$, where $u \in S$ and $v, w \notin S$ are non-adjacent vertices outside~$S$.
I verified computationally that the augmented bipartite graph (containment plus swap edges) between levels~$k$ and~$k+1$ satisfies Hall's condition for every tree on $n \le 18$ vertices (204\,909 trees, over 160\,000 level checks, zero failures).
By Hall's theorem, this gives an injection $i_k \hookrightarrow i_{k+1}$, confirming $i_k \le i_{k+1}$ for all $k$ below the mode.

The swap mechanism is guaranteed to exist by an analytic result.
Call a vertex $v \notin S$ \emph{private for~$u \in S$} if $u$ is the only $S$-neighbor of~$v$.
...
By pigeonhole, if $n \ge 3k$ then some $u \in S$ has $\mathrm{priv}(u) \ge 2$.
In a tree, two private neighbors of the same vertex are non-adjacent (since a triangle is impossible), so $(S \setminus \{u\}) \cup \{v, w\}$ is an independent set of size $k + 1$: a valid swap.
This covers 99.6\% of maximal independent sets below the mode through $n = 18$.

For the remaining 0.4\% (where $n < 3k$), a reduction argument applies.
If no vertex in~$S$ has $\mathrm{priv}(u) \ge 2$, then $P = \sum_{u \in S} \mathrm{priv}(u) \le k$, and the bound gives $k \ge (n + 1)/3$.
A computational search confirms that no tree with $\mathrm{mode}(I(T)) > \lfloor n/3 \rfloor + 1$ admits a maximal independent set in which every vertex has $\mathrm{priv} \le 1$ (verified exhaustively through $n = 18$; 99{,}147 maximal IS below the mode, zero failures).
Since $k \ge \lceil (n + 1)/3 \rceil = \lfloor n/3 \rfloor + 1 \ge \mathrm{mode}$ in the remaining case, the Private Neighbor Property holds universally through $n = 18$: every maximal independent set of size $k < \mathrm{mode}$ has some vertex with $\ge 2$ private neighbors.

Structurally, high-mode trees ($\mathrm{mode} > \lfloor n/3 \rfloor + 1$) are star-like: all have a vertex of degree $\ge 4$ with $\ge 2$ pendant leaf-neighbors (verified through $n = 20$).
When such a vertex belongs to~$S$, its leaf-neighbors are automatically private, giving $\mathrm{priv} \ge 2$; when it is excluded from~$S$, the subtree IS vertices typically inherit high private-neighbor counts.
An additional observation: $\mathrm{mode}(I(T)) \le \lfloor (n-1)/2 \rfloor$ for every tree (verified through $n = 20$, with equality at $K_{1,n-1}$ for odd~$n$).

Two gaps remain for a complete proof of unimodality via this route.
First, the absence of maximal IS with all $\mathrm{priv} \le 1$ below the mode in high-mode trees is verified only computationally; an analytic argument would close the Private Neighbor Property.
Second, proving that every independent set below the mode has augmented degree $\ge 1$ is necessary but not sufficient for Hall's condition; the full matching requires expansion properties of the augmented bipartite graph that remain unproved.

\subsection*{Remaining directions}

\begin{itemize}
  \item Prove analytically that high-mode trees ($\mathrm{mode} > \lfloor n/3 \rfloor + 1$) admit no maximal independent set in which every vertex has $\mathrm{priv} \le 1$.
  \item Establish Hall's condition for the augmented bipartite graph, or find an alternative route from individual swap existence to a full injection.
  \item A closed-form characterization of the optimal multi-arm configuration as a function of~$n$, and whether the number of arms grows without bound.
