...
Let $\broom(p, s)$ be a path on $p$ vertices with $s$ leaves attached at one end.
Applying the vertex-deletion recurrence at the attachment vertex gives the closed form
...
\begin{theorem}[Asymptotic broom unimodality]\label{thm:broom-asymptotic}
Fix $p \ge 2$ and write $A(x) = I(P_{p-1}; x)$ with
$\mu = A'(1)/A(1)$ and $m = \lceil \mu + \tfrac{1}{2} \rceil$.
Let $\nm(s)$ be the near-miss ratio of the broom $\broom(p, s)$.
Then, as $s \to \infty$,
\[
  \nm(s) \;=\; 1 - \frac{C(p)}{s} + O\!\left(\frac{1}{s^2}\right),
  \quad C(p) = (4m + 2) - 4\mu.
\]
In particular, $C(p) \in [4, 8)$, so $\nm(s) < 1$ for all sufficiently large $s$ and $\broom(p, s)$ is unimodal for all $s \ge s_0(p)$.
\end{theorem}

\begin{proof}
Let $B(x) = I(P_{p-2}; x)$ and write the closed form~\eqref{eq:broom-closed} as
$I(\broom(p, s); x) = (1+x)^s A(x) + x B(x)$.
Let $c_k = [x^k] I(\broom(p, s); x)$ and write $c_k = d_k + e_k$ with
\[
  d_k = \sum_{j=0}^{d} a_j \binom{s}{k-j}, \qquad
  e_k = [x^k]\, x B(x),
\]
where $A(x)=\sum_{j=0}^d a_j x^j$ and $d = \deg A$ depends only on $p$.
...
\]
since $j$ and $y$ are $O_p(1)$. Taking the weighted average yields
\[
...
$e_k$ is $O_p(1)$ while $d_k \asymp \binom{s}{\lfloor s/2 \rfloor}$, so
...
\]
Since $m - (\mu + \tfrac{1}{2}) \in [0, 1)$, we have $C(p) \in [4, 8)$, so
$\nm(s) < 1$ for all sufficiently large $s$.
\end{proof}

\begin{remark}
  The constant $C(p)$ depends on $p$ through $\mu = G_{p-1}/F_{p-1}$ and its ceiling.
  Since $\mu$ grows linearly in $p$ while $m = \lceil \mu + \tfrac{1}{2} \rceil$ is an integer, $C(p)$ oscillates between 4 and 8 but remains bounded away from zero.
  Proving $C(p) > 0$ for all $p$ would give an elementary proof of unimodality for the entire broom family; this is now subsumed by the spider log-concavity result of Li et al.\citep{li2025spiders}
\end{remark}
